...
# Instructions
## Part A: To select regions to target with SNIP-R, here are some important information to get started:
1. All code for target region selection are performed in R and R studio. Key packages used in this steps are: bedtoolsr from PhanstielLab, tidyverse, and readr. The exact packages used and version information are provided in the html output of the rmarkdown file in SNIP-R screen region selection folder.
2. Necessary dataset (Note: we included example datasets used in our paper as reference to replicate our result. As our paper focus on IFNG, located in human chromosome 12, most files only contain chromosome 12 information).
...
3. 00_ScreenRegionSelection markdown file contains all the code necessary for this step.

## Part B: To design sgRNA pairs for SNIP-R screen, here are some important information to get started:
...
## Part C: To analyse the result of SNIP-R screen, here are some important information to get started:
1. All code for target region selection are performed in R and R studio. Key packages used in this steps are: readr, tidyverse, readxl, ggrepel, and patchwork. The exact packages used and version information are provided in the html output of the rmarkdown file in SNIP-R screen analysis folder.
